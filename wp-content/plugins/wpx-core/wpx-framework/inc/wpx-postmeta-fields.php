...
if( !class_exists( 'WPX_Postmeta_Fields' ) ){

	class WPX_Postmeta_Fields {
		
		public function display_fields( $fields, $post_id, $post_status ){
			echo '<table class="wpx-postmeta-container">';
		...
		private function display_repeater_field( $key, $field, $post_id ){
			...
			echo '<tr><td colspan="2" class="wpx-postmeta-repeater-wrap" data-num="'.$count.'" data-fieldname="'. esc_attr( $key ) .'">';

			// First Hidden Item
			echo '<table class="wpx-postmeta-repeater repeater-init">';
			foreach ( $field['value'] as $key2 => $field2 ) {
			...
			if ( !empty( $meta ) ){
				foreach ( $meta as $item => $itemvalue ) {
					echo '<table class="wpx-postmeta-repeater">';
			...
			$buttontext = empty( $field['button'] ) ? esc_html__( 'Add More', 'wpx' ) : $field['button'];
			echo '<div class="wpx-postmeta-repeater-addmore"><button>'. $buttontext .'</button></div></td></tr>';
		}

		private function display_single_field( $key, $field, $post_id, $default = false ){
			$desc = '';
			if ( !empty( $field['desc'] ) ){
				$desc = '<div class="wpx-postmeta-desc">' . wp_kses_post( $field['desc'] ) . '</div>';
			}

			$container_attr = '';
			if ( !empty( $field['required'] ) ) {
				$container_attr .= ' class="wpx-postmeta-dependent"'; 
		...
		public function multi_select( $key, $field, $default, $class ){
			...
			echo '<select class="wpx-multi-select" data-placeholder=" ' . esc_attr__( 'Click here to select options', 'wpx' ) . '" multiple="multiple"'.
			' name="' . esc_attr( $key ) . '[]"'.
			' id="' . esc_attr( $key ) . '">';
			foreach ( $field['options'] as $key => $value ) {
				echo '<option',
		...
		public function multi_checkbox( $key, $field, $default, $class ){
			...
			foreach ( $field['options'] as $value => $title ) {
				...
				echo '<span class="wpx-postmeta-radio"><input type="checkbox"'. $class .
				' name="' . esc_attr( $key ) . '[]"'.
				' id="' . esc_attr( $id ) . '"'.
		...
		public function radio( $key, $field, $default, $class ){
			foreach ( $field['options'] as $value => $title ) {
				...
				echo '<span class="wpx-postmeta-radio"><input type="radio"'. $class .
				' name="' . esc_attr( $key ) . '"'.
				' id="' . esc_attr( $id ) . '"'.
				' value="' . esc_attr( $value ) . '"',
		...
		public function image( $key, $field, $default, $class ){
			...
			<div class="wpx_metabox_image">
				...
				<input class="wpx_upload_image upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Choose Image', 'wpx' ). '" />
				<div class="wpx_remove_image_wrap" style="'. esc_attr( $disstyle ) .'"><a href="#" class="wpx_remove_image button" >' . esc_html__( 'Remove Image', 'wpx' ). '</a></div>
			</div>
			';
		}
		...
		public function gallery( $key, $field, $default, $class ){
			...
			echo '
			<div class="wpx_metabox_gallery">
				<input name="'. esc_attr( $key ) .'" type="hidden" class="custom_upload_image" value="'. esc_attr( $default ) .'" />
				<div class="custom_preview_images">' . $img_html . '</div>
				<div class="wpx_metabox_gallery_buttons">
					<input class="wpx_upload_gallery upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Add/Edit Gallery Images', 'wpx' ). '" />
					<a href="#" style="'. esc_attr( $disstyle ) .'" class="wpx_remove_gallery button" >' . esc_html__( 'Remove All Images', 'wpx' ). '</a>
				</div>
			</div>
			';
		}
		...
		public function file( $key, $field, $default, $class ){
			...
			echo '
			<div class="wpx_metabox_file">
				<input name="'. esc_attr( $key ) .'" type="hidden" class="custom_upload_file" value="'. esc_attr( $default ) .'" />
				<input class="wpx_upload_file upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Choose File', 'wpx' ). '" />
				<div class="wpx_remove_file_wrap" style="'. esc_attr( $disstyle ) .'"><a href="#" class="wpx_remove_file button" >' . esc_html__( 'Remove File', 'wpx' ). '</a></div>
		...
		public function color_picker( $key, $field, $default, $class ){
			echo '<input type="text" class="wpx-metabox-picker wpx-metabox-colorpicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function date_picker( $key, $field, $default, $class ){
			$format = isset( $field['format'] ) ? $field['format'] : 'MM dd, yy';
			echo '<input type="text" data-format="' . $format . '" class="wpx-metabox-picker wpx-metabox-datepicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function time_picker( $key, $field, $default, $class ){
			echo '<input type="text" class="wpx-metabox-picker wpx-metabox-timepicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function time_picker_24( $key, $field, $default, $class ){
			echo '<input type="text" class="wpx-metabox-picker wpx-metabox-timepicker-24" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}
