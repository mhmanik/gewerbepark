<?php
require_once 'wpx-postmeta-fields.php';

if( !class_exists( 'WPX_Postmeta' ) ){

	class WPX_Postmeta {
		protected static $instance = null;
		public  $version = WPX_FRAMEWORK_VERSION;
		...
		private $nonce_action = 'wpx_metabox_nonce';
		private $nonce_field = 'wpx_metabox_nonce_secret';

		private function __construct() {
			$this->fields_obj = new WPX_Postmeta_Fields();
		...
		public function load_styles_and_scripts(){
			wp_enqueue_style( 'wpx-posts-jqui', $this->base_url . 'assets/css/jquery-ui.css', array(), $this->version ); // only datepicker
			...
			wp_enqueue_style( 'wpx-posts-style', $this->base_url . 'assets/css/style.css', array(), $this->version );
			...
			wp_enqueue_media();
			wp_enqueue_script('wpx-posts-script', $this->base_url . 'assets/js/script.js', array('jquery', 'jquery-ui-core', 'wp-color-picker', 'jquery-ui-datepicker'), $this->version );
		}
		...
		public function add_meta_box( $id, $title, $post_types, $callback = '', $context = '', $priority = '', $fields = '' ) {
			...
			$fields = apply_filters( 'wpx_postmeta_field_' . $id, $fields );
			$metaboxes = array(
				'title'         => $title,
				'callback'      => $callback,
			...
			$this->metaboxes[$id] = apply_filters( 'wpx_metabox_' . $id, $metaboxes );
